...
module.exports = app => {
  const defaultConfig = {
    ...
    logging(...args) {
      ...
      app.logger.info('[egg-sequelize-module]%s %s', used, args[0]);
    },
    host: 'localhost',
    port: 3306,
    username: 'root',
  ...
  const config = app.config.sequelizeModule;
  ...
  function loadDatabase(config = {}) {
    if (typeof config.ignore === 'string' || Array.isArray(config.ignore)) {
      app.deprecate(`[egg-sequelize-module] if you want to exclude ${config.ignore} when load models, please set to config.sequelize.exclude instead of config.sequelize.ignore`);
    ...
    if (model[delegate[len - 1]]) {
      throw new Error(`[egg-sequelize-module] app[${config.delegate}] is already defined`);
    }
    ...
    /*** HERE IS THE MODIFICATION FOR EGGULAR */
    // const modelDir = path.join(app.baseDir, 'app', config.baseDir);
    const modelDir = [
      path.join(app.baseDir, 'app', config.baseDir),
      // handle module's model dir
      ...(
        app.loader.getLoadUnits()
          .filter(u => u.type === 'module')
          .map(u => path.join(u.path, 'app', config.baseDir))
      )
    ];
